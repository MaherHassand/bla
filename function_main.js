	...
	$.ajax({
		...
		success: function(response){
			if('check' in response){
				...
				var pvStatus = response.pvstatus; //message seeen
				speed = response.spd;
				inOut = response.acd;
				$('#gold').html(response.gold);
				if(response.pcrm == response.curp){
					$(".typing_text").remove();
					$("#private_name").append('<p class="typing_text text_xsmall bellips">بكتب الآن...</p>');
				} else {
				    $(".typing_text").remove();
				}
				...
				if(response.act != userAction ){
					...
					if(response.curp == $('#get_private').attr('value')){
					    if(pvStatus == response.curp){
							$("#private_content .pdate .fa-check").remove();
							$("#private_content .pdate").prepend('<i class="fa fa-check"></i><i class="fa fa-check"></i>'); 
						}
						if(privReload == 1){
							if(pLogs == ''){
...
userReload = function(type){
	if($('#container_user:visible').length || type == 1 || firstPanel == ''){
		if(type == 1){
			panelIt(0);
		}
...
$(document).ready(function(){
	...
	adjustPanelWidth();
	userlist = setInterval(userReload, 40000);
