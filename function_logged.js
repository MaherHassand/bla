...
showHatsapp = function(){
	$.post('system/box/whatsapp.php', {
			token: utk,
		}, function(response) {
			showModal(response, 500);
	});
...
showRankapp = function(){
	$.post('system/box/rankapp.php', {
			token: utk,
		}, function(response) {
			showModal(response, 500);
	});
...
showVerification = function(){
	$.post('system/box/verification.php', {
			token: utk,
		}, function(response) {
			showModal(response, 500);
	});
...
addRoom = function(){
	...
	sendPmToAll = function(){
		var whom = $('#set_whom').val();
		var text = $('#set_text').val();
		if(whom == '' || text == ''){
			callSaved(system.emptyField, 3);
			event.preventDefault();
		}
		else {
			$.post('system/action_pmToAll.php', { 
				whom: $('#set_whom').val(),
				text: $('#set_text').val(),
				token: utk
				}, function(response) {
					if(response == 0){
						...
						callSaved("تم ارسال الرسالة بنجاح", 1);
						hideOver();
					}
					else if(response == 2){
						callSaved(system.restrictedContent, 3);
						...
						callSaved("يجب أن تحتوي رسالتك على 5 أحرف على الأقل", 3);
					}
					else {
						return false;
					}
